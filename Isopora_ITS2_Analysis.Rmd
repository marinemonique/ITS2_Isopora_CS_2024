...
load("~/AIMS@JCU/CoralSeaSymbioants/ITS2 Isopora/kdist_iso_symbioant.RData")
```
...
```{r}
load("~/AIMS@JCU/CoralSeaSymbioants/ITS2 Isopora/Isopora/ind_coords_iso.RData")
...
```{r}
load("~/AIMS@JCU/CoralSeaSymbioants/ITS2 Isopora/Isopora/iso_gen.RData") #load host data
...
iso_gen_subset <- iso_gen[ind_names_host %in% sample_names, ]

length(indNames(iso_gen))          # Before
length(indNames(iso_gen_subset))   # After
...
hp.df.iso2 <- hp.df.iso %>% left_join(meta.reef) %>% 
  mutate(Reef = factor(Reef, levels = c("Elizabeth", "Middleton", "Cato", "Wreck", "Kenn", "Mellish", "Marion","Frederick", "Saumarez", "Madgelaine", "NA"))) 
