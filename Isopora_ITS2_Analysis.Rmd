...
```{r non-profile sequences}
nonprofileseqs <- all_data %>% 
  filter(str_detect(name, "non")) %>% 
  #filter(str_detect(name, "p_")) |> 
  summarise(total_seqs = sum(value)) %>%
  pull(total_seqs)
#2528918 non profile sequences maybe?
```
...
all_data %>%
  ...
  write.csv(file = "Isopora_ITS2_HostCluster_TP.csv")
```
...
load("~/AIMS@JCU/CoralSeasymbionts/ITS2_Isopora_CS_2024/kdist_iso_symbiont.RData")
```
```{r eval = FALSE, echo=FALSE}
#filter out non-profile sequences
seq_data <- all_data %>% 
...
```{r eval = FALSE, echo=FALSE}
seqs_wide_iso_symbiont <- seq_data %>%
  dplyr::select(sample_name, name, value) %>%
  pivot_wider(names_from = name, values_from = value, values_fill = 0) %>%
  tibble::column_to_rownames(var = "sample_name")

head(seqs_wide_iso_symbiont)
save(seqs_wide_symbiont, file =  "seqs_wide_symbiont.RData")
```

```{r eval = FALSE, echo=FALSE}
k_unidist_iso_D <- GUniFrac(seqs_wide_iso_symbiont, k_tree_iso_symbiont)   #GUniFrac calculates all the distances 
...
# Cluster the samples
hclust_samps_iso_symbiont <- upgma(du_iso_symbiont)

# Make the sample tree
tree_iso_symbiont <- ggtree(hclust_samps_iso_symbiont, size = 0.2) +
  theme(aspect.ratio = 0.3) + layout_dendrogram()

# Get a sample order from ggtree
iso_sample_order <- tree_iso_symbiont$data %>% filter(isTip == "TRUE") %>%
  arrange(y) %>%
  pull(label)

# Start plotting the composition data
plot_df_iso_symbiont <- all_data %>%
  mutate(sample_name = fct_relevel(sample_name, iso_sample_order))
...
## Fig 2B
```{r eval = FALSE, echo=FALSE}
bar_uni_iso_symbiont <- 
ggplot(plot_df_iso_symbiont, aes(sample_name, value_rel)) +
geom_bar(stat = "identity", aes(fill = name, colour = name)) +
theme(aspect.ratio = 0.5, legend.position = "none", axis.text.y=element_blank(), axis.ticks.y = element_blank(),
      axis.text.x=element_blank(), axis.ticks.x = element_blank(),
      axis.title.x = element_blank(), axis.title.y = element_blank(),
...
tree_iso_symbiont / bar_uni_iso_symbiont
```


```{r eval = FALSE, echo=FALSE}
# Create reef strip
reef_strip <- ggplot(plot_df_iso_symbiont, aes(x = sample_name, y = 1, fill = Reef)) +
  geom_tile() +
  theme_void() +
  theme(legend.position = "top") #+
...
# Now combine the reef strip and your barplot
combined_plot <- reef_strip / bar_uni_iso_symbiont + plot_layout(heights = c(0.1, 1))
...
symbiont_iso_all <- all_data
save(symbiont_iso_all, file = "symbiont_iso_all.RData")
```
...
```{r eval = FALSE, echo=FALSE}
load("~/AIMS@JCU/CoralSeasymbionts/ITS2_Isopora_CS_2024/Isopora/iso_gen.RData") #load host data
...
#must have same number of dimensions 
dim(H.dist)
dim(P.dist) #its really odd that the symbiont has 186 when the code running above has 187 sample names

# Subset to only the samples that are in both datasets
intersect_samples <- intersect(rownames(H.dist), rownames(P.dist))
intersect_samples_col <- intersect(colnames(H.dist), colnames(P.dist)) # The host data set had 189, and the symbiont had 186
P.dist_intersect <- P.dist[intersect_samples, intersect_samples_col]
H.dist_intersect <- H.dist[intersect_samples, intersect_samples_col ]
