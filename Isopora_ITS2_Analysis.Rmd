...
seqs_long_c <- seqs_long %>%  filter(!str_detect(name, "non")) %>% 
...
all_data <- rbind(seqs_long, profile_data) #need to work out why my profiles_data has 51 variables, but the seqs_long only has 50.
...
sym_bar <- ggplot(symbiont_long, aes(x = sample_name, y = rel_abund, fill = name)) +
  ...
  facet_wrap(~HostCluster, scales = "free_x", nrow = 1) +
  theme_bw() +
  labs(
    x = "Sample",
...
## Moniques Code of the same thing above - Just combined into a single R markdown document.
```{r, warning = FALSE, echo = FALSE, message=FALSE}
library(ggsci)
library(tidyverse)
library(dplyr)
